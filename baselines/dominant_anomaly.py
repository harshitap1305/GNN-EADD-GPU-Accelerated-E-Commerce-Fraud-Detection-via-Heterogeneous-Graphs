...
def load_hetero_data(meta_path, review_path):
    ...
    # Reverse edges so buyer & seller become destination nodes → get embeddings
...
def run_dominant():
    ...
    print(f"Executing DOMINANT on: {device}")
    ...
    alpha = 0.7
    ei    = data['buyer', 'reviews', 'product'].edge_index   # [2, E]
    ...
    with torch.no_grad():
        ...
        # Product anomaly score: attribute reconstruction error
        prod_scores = torch.norm(x_hat - data['product'].x, dim=1).cpu().numpy()

        # Buyer anomaly score: mean product-score over all products they reviewed
    ...
    # ── Three-sigma thresholds ──────────────────────────────────────────
    thresh_prod   = np.mean(prod_scores)  + 3 * np.std(prod_scores)
    ...
    # ══════════════════════════════════════════════════════════════════
    #  OUTPUT 1 — Metadata anomalies
    #  anomalies_metadata.csv        : asin, anomaly_score
    #  anomalous_asins_metadata.txt  : one ASIN per line
    # ══════════════════════════════════════════════════════════════════
    meta_rows = [{'asin': idx_to_asin[int(i)],
                  'anomaly_score': round(float(prod_scores[i]), 6)}
                 for i in anom_prod_idx]
    df_meta = (pd.DataFrame(meta_rows)
                 .sort_values('anomaly_score', ascending=False)
                 .reset_index(drop=True))
    df_meta.to_csv('anomalies_metadata.csv', index=False)
    with open('anomalous_asins_metadata.txt', 'w') as f:
        f.write('\n'.join(df_meta['asin'].tolist()))

    # ══════════════════════════════════════════════════════════════════
    #  OUTPUT 2 — 5-Core anomalies
    #  anomalies_5core_buyers.csv  : reviewerID, anomaly_score
    #  anomalous_asins_5core.txt   : ASINs reviewed by anomalous buyers
    # ══════════════════════════════════════════════════════════════════
    review_rows = [{'reviewerID':    idx_to_buyer[int(i)],
                    'anomaly_score': round(float(buyer_scores[i]), 6)}
                   for i in anom_buyer_idx]
    df_buyers = (pd.DataFrame(review_rows)
                   .sort_values('anomaly_score', ascending=False)
                   .reset_index(drop=True))
    df_buyers.to_csv('anomalies_5core_buyers.csv', index=False)

    # Collect ASINs that anomalous buyers reviewed
    ...
    with open('anomalous_asins_5core.txt', 'w') as f:
    ...
    print(f"[Metadata]  → anomalies_metadata.csv")
    print(f"[Metadata]  → anomalous_asins_metadata.txt")
    print()
    ...
    print(f"[5-Core]    → anomalies_5core_buyers.csv")
    print(f"[5-Core]    → anomalous_asins_5core.txt")
    print("=" * 58)
